...
def _run_in_container(target_root, command_list, description, progress_callback=None, timeout=None, pipe_input=None):
    ...
    nspawn_cmd = [
        \"systemd-nspawn\", 
        \"-q\", # Maybe remove -q temporarily for debugging?
        f\"-D{target_root}\",
        \"--capability=all\", 
        \"--bind-ro=/etc/resolv.conf\",
        # Bind essential pseudo-filesystems if needed (nspawn often does this, but explicit can help)
        # \"--bind=/dev\", \"--bind=/proc\", \"--bind=/sys\",
        # \"--bind=/tmp\",
    ] + command_list
    
    success, err, stdout = _run_command(nspawn_cmd, description, progress_callback, timeout, pipe_input)
    
    # Add a bit more context to the error if possible
    if not success and not err:
        err = f\"{description} failed within container (systemd-nspawn) but no specific error captured.\"
    elif not success and err:
        err = f\"{description} failed within container (systemd-nspawn): {err}\"
        
    return success, err # Return only success/err for compatibility
...
def create_user_in_container(target_root, user_config, progress_callback=None):
    \"\"\"Creates user account with hashed password in target via systemd-nspawn.\"\"\"
    username = user_config.get('username')
    # Get the hashed password from config
    hashed_password = user_config.get('hashed_password', None) 
    ...
    if not hashed_password:
         # This should not happen if UserPage validation works
         return False, \"Hashed password not provided for user creation.\"

    # Build useradd command with -p option for hashed password
    useradd_cmd = [\"useradd\", \"-m\", \"-s\", \"/bin/bash\", \"-U\"]
    useradd_cmd.extend([\"-p\", hashed_password]) # Add hashed password
    if real_name:
...
def install_packages_dnf(target_root, progress_callback=None):
    ...
    # Refined package set including sudo
    packages = [
...
def generate_fstab(target_root, disk_config, progress_callback=None):
    \"\"\"Generates /etc/fstab for the installed system using UUIDs.\"\"\"
    print(\"Generating /etc/fstab...\")
    if progress_callback:
        progress_callback(\"Generating /etc/fstab...\")
        
    partitions = disk_config.get(\"partitions\", [])
    if not partitions:
        return False, \"No partition information found to generate fstab.\"
        
    fstab_lines = [\"# /etc/fstab: static file system information.\", \"#\"]
    fstab_lines.append(\"# <file system> <mount point>   <type>  <options>       <dump>  <pass>\")
    
    # Get UUIDs for partitions
    uuids = {}
    devices_to_check = [p[\"device\"] for p in partitions if \"device\" in p]
    if not devices_to_check:
         return False, \"No device paths found in partition information.\"
         
    # Use blkid to get UUIDs. Needs to run on host, not container.
    # Temporarily modify _run_command signature here for clarity, 
    # but ideally refactor _run_command or add a non-pkexec variant.
    def run_host_command(cmd_list, desc, timeout=15):
        print(f\"Running on host: {desc} -> {' '.join(shlex.quote(c) for c in cmd_list)}\")
        try:
            ...
            err = f\"Host command not found: {cmd_list[0]}\"
            print(f\"ERROR: {err}\")
            return False, err, None
        except subprocess.CalledProcessError as e:
            err = f\"Host command {desc} failed: {e.stderr.strip() or f'Exit code {e.returncode}'}\"
            print(f\"ERROR: {err}\")
            return False, err, None
        except subprocess.TimeoutExpired:
            err = f\"Host command {desc} timed out after {timeout}s.\"
            print(f\"ERROR: {err}\")
            return False, err, None
        except Exception as e:
            err = f\"Unexpected error running host command {desc}: {e}\"
            print(f\"ERROR: {err}\")
            return False, err, None

    blkid_full_cmd = [\"blkid\"] + devices_to_check
    # Run blkid directly on the host
    success, err, blkid_output = run_host_command(blkid_full_cmd, \"Get Partition UUIDs\")
    if not success:
         return False, f\"Failed to get blkid output: {err}\"
         
    # Parse the full blkid output
    for line in blkid_output.strip().split('\\n'):
        if ':' in line and 'UUID=' in line:
             device_part = line.split(':')[0]
             uuid_match = re.search(r'UUID=\"([^\"]+)\"', line)
             if uuid_match:
                 uuids[device_part] = uuid_match.group(1)
    ...
    for part_info in partitions:
        ...
        if not device or not mountpoint or not fstype:
            print(f\"Warning: Skipping fstab entry for incomplete partition info: {part_info}\")
            continue
            
        uuid = uuids.get(device)
        if not uuid:
             # Partitions might take time to settle after mkfs
             print(f\"Warning: Could not find UUID for device {device} initially. Retrying blkid...\")
             time.sleep(3) # Wait 3 seconds
             success, err, blkid_output_retry = run_host_command([\"blkid\", device], f\"Retry Get UUID for {device}\")
             if success and blkid_output_retry:
                  uuid_match = re.search(r'UUID=\"([^\"]+)\"', blkid_output_retry)
                  if uuid_match:
                      uuid = uuid_match.group(1)
                      uuids[device] = uuid # Store for potential future use
                      print(f\"  Found UUID {uuid} on retry.\")
             if not uuid:
                 print(f\"ERROR: Still Could not find UUID for device {device} after retry. Skipping fstab entry.\")
                 # Consider this a fatal error?
                 # return False, f\"Failed to find UUID for {device}\" 
                 continue # Skip entry for now
             
        # Define default options based on mount point
        if mountpoint == \"/\":
            options = \"defaults\"
            dump = 1
            pass_num = 1
            ...
            # Add rules for /boot, swap, /home etc. if needed
            options = \"defaults\"
            dump = 0
            pass_num = 2
            
        fstab_line = f\"UUID={uuid}  {mountpoint}  {fstype}  {options}  {dump} {pass_num}\"
    ...
    try:
        ...
        fstab_content = \"\\n\".join(fstab_lines) + \"\\n\"
        write_cmd = [\"tee\", fstab_path]
        success, err, _ = _run_command(write_cmd, \"Write /etc/fstab\", progress_callback, timeout=15, pipe_input=fstab_content)
        if not success:
            return False, f\"Failed to write {fstab_path} via tee: {err}\"
            
        print(f\"Successfully wrote {fstab_path}\")
        return True, \"\"
    except Exception as e:
        err = f\"Failed to write {fstab_path}: {e}\"
        print(f\"ERROR: {err}\")
        return False, err 